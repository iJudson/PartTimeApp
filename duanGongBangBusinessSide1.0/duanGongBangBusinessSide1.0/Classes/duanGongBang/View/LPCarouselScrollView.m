...
#import "LPCarouselScrollView.h"
#import "LPWebImageManager.h"

@interface LPCarouselScrollView () <UIScrollViewDelegate>

@property (nonatomic,copy) NSArray *imageData;

@end

@implementation LPCarouselScrollView{
...
+ (instancetype)picScrollViewWithFrame:(CGRect)frame WithImageUrls:(NSArray<NSString *> *)imageUrl {
    return  [[LPCarouselScrollView alloc] initWithFrame:frame WithImageNames:imageUrl];
}
...
- (UIImage *)setImageWithIndex:(NSInteger)index {
    ...
    //从内存缓存中取,如果没有使用占位图片
    UIImage *image = [[[LPWebImageManager shareManager] webImageCache] valueForKey:_imageData[index]];
...
- (void)getImage {
    ...
    for (NSString *urlSting in _imageData) {
        [[LPWebImageManager shareManager] downloadImageWithUrlString:urlSting];
    }
